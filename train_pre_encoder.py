...
class RealMelSpectrogramDataset(Dataset):
    def __init__(self, data_dir):
        # Expect a folder "real" inside data_dir containing .npy files recursively
    ...
    def __getitem__(self, idx):
        ...
        if real_spectrogram.ndim != 2:
             print(f"Warning: Spectrogram {real_path} has unexpected shape {real_spectrogram.shape}. Skipping or using dummy.")
...
# =============================================================================
def plot_mel_spectrogram(spectrogram, vmin, vmax, save_path, title='Mel Spectrogram', ylabel='Frequency', xlabel='Time'):
    """
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, batch_data in enumerate(loop):
            ...
            with autocast(enabled=True): # Use enabled=True for clarity, default is True if torch.cuda.is_available()
                ...
                mask_time = torch.arange(max_mel_len, device=device).expand(len(mel_lens), max_mel_len) < mel_lens.unsqueeze(1)
        ...
        if real_spectrograms is not None and real_spectrograms.size(0) > 0 and recon_spectrograms is not None:
            ...
            for i in range(num_plots):
                ...
                plot_mel_spectrogram(
                    ...
                    save_path=os.path.join(plot_dir, f'epoch_{epoch+1:03d}_train_orig_{i+1}_{os.path.splitext(original_filename)[0]}.png'),
                    title=f'Epoch {epoch+1} - Original {i+1} ({original_filename})'
                )
                plot_mel_spectrogram(
                    recon_spec, vmin=vmin, vmax=vmax,
                    save_path=os.path.join(plot_dir, f'epoch_{epoch+1:03d}_train_recon_{i+1}_{os.path.splitext(original_filename)[0]}.png'),
            ...
            with torch.no_grad():
                ...
                for i, idx in enumerate(eval_indices):
                    ...
                    plot_mel_spectrogram(
                        ...
                        save_path=os.path.join(plot_dir, f'epoch_{epoch+1:03d}_eval_orig_{i+1}_{os.path.splitext(filename)[0]}.png'),
                    ...
                    if reconstructed is not None and reconstructed.numel() > 0 :
                        plot_mel_spectrogram(
                            ...
                            vmin=vmin_eval, vmax=vmax_eval,
                            save_path=os.path.join(plot_dir, f'epoch_{epoch+1:03d}_eval_recon_{i+1}_{os.path.splitext(filename)[0]}.png'),
...
def main():
    ...
    # Data args
    parser.add_argument('--data_dir', type=str, required=True, help='Root directory containing the "real" subdirectory with .npy spectrograms.')
    parser.add_argument('--output_dir', type=str, default='training_output', help='Directory to save plots and checkpoints.')
    parser.add_argument('--batch_size', type=int, default=16, help='Training batch size.')
    parser.add_argument('--num_workers', type=int, default=0, help='Number of workers for DataLoader.')
    parser.add_argument('--validation_split', type=float, default=0.1, help='Fraction of data to use for validation/evaluation plotting (0.0 to 1.0).')

    # Model args
    parser.add_argument('--mel_channels', type=int, default=88, help='Number of mel frequency channels in the input spectrograms.')
    parser.add_argument('--latent_dim', type=int, default=1024, help='Dimension of the latent space in the autoencoder.')
    ...
    parser.add_argument('--eval_interval', type=int, default=2, help='Frequency (in epochs) to perform evaluation plotting.')
    parser.add_argument('--save_interval', type=int, default=5, help='Frequency (in epochs) to save model checkpoints.')
    parser.add_argument('--num_plot_examples', type=int, default=3, help='Number of examples to plot during training and evaluation.')
    ...
    try:
        ...
        if train_size <= 0 or eval_size < 0: # Eval size can be 0 if split is 0.0
             print(f"Error: Invalid train/eval split sizes. Train: {train_size}, Eval: {eval_size}. Check validation_split.")
             return
    ...
    autoencoder = PreEncoder(mel_channels=args.mel_channels, channels= [1024, 1024], kernel_sizes=[11, 5]).to(device)
